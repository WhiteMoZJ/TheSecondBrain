...
- $(x^{(i)},y^{(i)})$ 称为**训练示例**
- $m$ 个训练示例的集合$(x^{(i)},y^{(i)});i=1,...,m$ 称为**训练集training set**
...
假设有 $m$ 个训练示例，所有 $x^{(i)}$ 对应的误差的平方均值，通常会再 $÷2$ ，即
$$
J(w,b)=\frac{1}{2m}\sum^{m}_{i=1}(f_{w,b}(x^{(i)})-y^{(i)})^2
$$
称之为**代价函数**
![Python实现代价函数](机器学习Python源码.md##代价函数)
...
实现梯度下降
![向量化提高效率](机器学习Python源码.md#梯度下降##梯度下降)
```ad-info
以 $w$ 参数为例
...
- $x_j$ 表示第 $j$ 个特征
-  $n$ 表示特征总数
- $\vec{x}^{(i)}$ 表示第 $i$ 个训练示例的特征集
- $x^{(i)}_j$ 表示第 $i$ 个训练示例的第 $j$ 个特征
用向量表示
$$
\begin{gather}
\vec{w}=\begin{bmatrix}
...
f(x)=1/(1+E^(-x))
```
`sigmod`函数的输入 $z$ 就是所有训练示例的线性回归函数模型
$$
\begin{gather}
